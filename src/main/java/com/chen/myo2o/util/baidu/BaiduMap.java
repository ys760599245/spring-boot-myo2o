...
import com.chen.myo2o.util.weixin.WeixinUtil;
...
public class BaiduMap {
    ...
    public static Point antiGeocoder(double lat, double lng) {
        ...
        url += "ak=" + ak + "&callback=renderReverse&location=" + lat + "," + lng + "&output=" + outPut + "&pois="
                + pois + "&coordtype=" + coorType;
        JSONObject object = WeixinUtil.httpRequest(url, "GET", null);
        if (object != null) {
            JSONObject result = object.getJSONObject("result");
     ...
     */
    public static List<Distance> getDistance(Point origins, List<Point> destinations, String mode) {
        String url = "http://api.map.baidu.com/direction/v1/routematrix?";
        url += "output=json&ak=" + ak + "&origins=" + origins.getLat() + "," + origins.getLng() + "&destinations=";
        ...
        if (object != null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                dis = new Distance(distance.getString("text"), distance.getInt("value"), duration.getString("text"),
     ...
     * 生成静态百度地图
     */
    public static String staticMap(Point origin, Point destination, int width, int height) {
        ...
        url += "ak=" + ak + "&width=" + width + "&height=" + height + "&center=" + centerLng + "," + centerLat
                + "&scale=2&zoom=15";
        url += "&markers=" + origin.getLng() + "," + origin.getLat() + "|" + destination.getLng() + ","
                + destination.getLat();
        return url;
    }
